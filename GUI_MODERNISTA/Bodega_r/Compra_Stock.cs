...
namespace Diseno
{
    public partial class Compra_Stock : MaterialForm
    {
        ...
        {
            Cargar_proveedores_orden(TxtProveedor.Text);
        }
        private void Cargar_proveedores_orden(string v_nombre)
        {
            Acceso ac = new Acceso();
            var conn = ac.conectar();
            conn.Open();
            ...
            DataTable dt = new DataTable();
            string query = "select id_proveedor,razon_social from proveedor where razon_social like '%"+v_nombre+"%'";
            OracleCommand command = new OracleCommand(query, conn);
            OracleDataAdapter da = new OracleDataAdapter(command);
            da.Fill(dt);
